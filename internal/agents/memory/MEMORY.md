...
- Docs publishing installs `docs/requirements.txt` before `requirements.txt` with pip hash checking enabled, builds Markdown with Sphinx, and publishes to the `main` branch of the central docs repository.
...
- The public vBase SDK dependency is installed from PyPI as `vbase==1.0.0`, not from the `validityBase/vbase-py` Git repository.
