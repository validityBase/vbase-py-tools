...
- Docs publishing installs `docs/requirements.txt` with pip hash checking enabled, builds Markdown with Sphinx, and publishes to the `main` branch of the central docs repository.

## Python Dependencies
- Runtime package dependencies are declared as ranges in `requirements.in`; `setup.py` reads this input file for package metadata.
